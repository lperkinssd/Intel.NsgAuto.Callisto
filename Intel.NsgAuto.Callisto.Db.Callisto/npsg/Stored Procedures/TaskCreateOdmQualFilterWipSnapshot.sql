...
-- Description : Create records in [npsg].[OdmWipSnapshots] from the latest data in
--               [TREADSTONENPSGPRD].[treadstone].[odm].[vw_callisto_odm_wip_data]
-- Notes       : This must be called before [npsg].[TaskCreateOdmQualFilterLotShipSnapshot]
--               due to a dependency. Copied and tweaked from [npsg].[CreateOdmWipSnapshotDaily]
-- Example     : EXEC [npsg].[TaskCreateOdmQualFilterWipSnapshot];
-- ================================================================================================
CREATE PROCEDURE [npsg].[TaskCreateOdmQualFilterWipSnapshot]
(
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC [CallistoCommon].[stage].[CreateTaskByName] @TaskId OUTPUT, 'Create ODM QF NPSG WIP Snapshot';
		DECLARE @Count         INT;
		DECLARE @Version       INT = (SELECT ISNULL(MAX([Version]), 0) FROM [npsg].[OdmWipSnapshots] WITH (NOLOCK));
		...
		FROM [TREADSTONENPSGPRD].[treadstone].[odm].[vw_callisto_odm_wip_data] ows WITH (NOLOCK)
		...
		BEGIN
			...
			EXEC [CallistoCommon].[stage].[UpdateTaskProgress] @TaskId, 90, 'Creating a new version and inserting records into [npsg].[OdmWipSnapshots]';
			...
			INSERT INTO [npsg].[OdmWipSnapshots]
			SELECT DISTINCT
				  @Version
				, [media_lot_id]
			...
			SET @MessageText = 'Version ' + CAST(@Version AS VARCHAR(20)) + ' created in [npsg].[OdmWipSnapshots] with number of records: ' + CAST(@CountInserted AS VARCHAR(20));
			EXEC [CallistoCommon].[stage].[CreateTaskMessage] @TaskId, 'Information', @MessageText;
