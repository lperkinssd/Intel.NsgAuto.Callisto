...
-- =============================================
CREATE PROCEDURE [npsg].[ApproveMATVersion] 
...
BEGIN
	...
	--IF (@Count > 0 AND @NewStatusId IS NOT NULL AND ((@CurrentStatus IN ('Submitted', 'In Review') AND @UserId IN (SELECT [Idsid] FROM [npsg].[MATReviewers] WHERE [VersionId] = @Id AND [ReviewStageId] = @StageId AND [ReviewGroupId] = @GroupId)) OR @Override = 1))
	IF (@Count > 0 AND @NewStatusId IS NOT NULL AND ((@CurrentStatus = 'Submitted' AND @UserId IN (SELECT [Idsid] FROM [npsg].[MATReviewers] WHERE [VersionId] = @Id AND [ReviewStageId] = @StageId AND [ReviewGroupId] = @GroupId)) OR @Override = 1))
	BEGIN
	...
	MERGE [npsg].[MATReviewDecisions] AS TRG
	...
	FROM [npsg].[MATReviewers] RR
		INNER JOIN [npsg].[MATReviewGroups] G (NOLOCK) ON (G.Id=RR.ReviewGroupId)
		INNER JOIN [npsg].[MATReviewStages] S (NOLOCK) ON (S.Id=RR.ReviewStageId)
		WHERE RR.Id=@ReviewerId

	DECLARE @Audit varchar(100) = (SELECT 'Review Approved. Stage=' + @StageName + ', Group=' + @GroupName + ' Comment: ' + @Comment)

	EXEC [npsg].[CreateMATChangeHistory] @Id, @Audit, @UserId
	...
	DECLARE @ApproveCount int = (SELECT COUNT(*) FROM [npsg].[MATReviewDecisions] WHERE [VersionId] = @Id AND [IsApproved] = 1);
	DECLARE @IsPOR bit = 0;
	...
	EXEC [npsg].[GetMATCurrentStage] @Id, @UserId, @CurrentStageId OUTPUT, @CurrentStageName OUTPUT
	...
	WHERE [VersionId] = @Id
	AND   [Idsid] = (SELECT MAX([ChangedBy]) FROM [npsg].[MATReviewChangeHistory] WHERE [VersionId] = @Id AND [Description] = 'Request Submitted');
