...
--					any emails.  This SP uses the npsg.MATReviewStageStates table
...
--					[npsg].[GetMATReviewersWithWork] stored proc to get the emails.  This
...
--
--					EXEC [npsg].[GetMATCurrentStagesForEmailing] @VersionId, @UserId, @IsUpdate, @StageId1 OUTPUT, @StageId2 OUTPUT
--
-- =============================================
CREATE PROCEDURE [npsg].[GetMATCurrentStagesForEmailing] 
...
BEGIN
 ...
 --   DROP TABLE #LEVEL3
	...
	INTO #LEVEL3
	FROM #LEVEL2
	LEFT JOIN [npsg].[MATReviewStageStates] S WITH (NOLOCK) ON (S.[VersionId]=@VersionId AND S.[ReviewStageId]=[SID])
	...
	IF (@MinSeq > 0) BEGIN
		...
		IF (@IsUpdate = 1) BEGIN
			UPDATE [npsg].[MATReviewStageStates] 
			SET [HasEmailSent]=0 
		...
		IF (@IsActive IS NULL OR  @IsActive != 1) BEGIN
			...
			INSERT INTO [npsg].[MATReviewStageStates] 
		...
		IF (@TotalRejected > 0) BEGIN
			...
			FROM [npsg].[MATReviewStageStates] WITH (NOLOCK) 
			WHERE [VersionId]=@VersionId 
			AND [ReviewStageId]=@SID
