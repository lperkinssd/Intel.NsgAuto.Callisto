...
-- Description : Create records in [npsg].[OdmLotShipSnapshots] from the latest data in
--               [TREADSTONENPSGPRD].treadstone.[dbo].[lot_ship]
-- Note:         This must be called after [npsg].[TaskCreateOdmQualFilterLotShipSnapshot]
--               due to dependency on [npsg].[OdmWipSnapshots]. Copied and tweaked from
--               [npsg].[CreateOdmLotShipSnapshotDaily]
-- Example     : EXEC [npsg].[TaskCreateOdmQualFilterLotShipSnapshot];
-- ======================================================================================
CREATE PROCEDURE [npsg].[TaskCreateOdmQualFilterLotShipSnapshot]
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC [CallistoCommon].[stage].[CreateTaskByName] @TaskId OUTPUT, 'Create ODM QF NPSG Lot Ship Snapshot';
		...
		DECLARE @Count       INT;
		DECLARE @WipVersion  INT = (SELECT MAX([Version]) FROM [npsg].[OdmWipSnapshots]      WITH (NOLOCK));
		DECLARE @Version     INT = (SELECT MAX([Version]) FROM [npsg].[OdmLotShipSnapshots]  WITH (NOLOCK));
		...
		FROM [TREADSTONENPSGPRD].treadstone.[dbo].[lot_ship] AS ls WITH (NOLOCK)
		INNER JOIN [npsg].[OdmWipSnapshots] AS owip WITH (NOLOCK)
		...
		SELECT
			...
			, [fab_conv_id]
			, [shipping_label_lot]
		FROM [npsg].[OdmLotShipSnapshots] WITH (NOLOCK)
		...
		IF (@Count > 0)
		BEGIN
			EXEC [CallistoCommon].[stage].[UpdateTaskProgress] @TaskId, 75, 'Creating a new version and inserting into [npsg].[OdmLotShipSnapshots]';
			...
			INSERT INTO [npsg].[OdmLotShipSnapshots]
			(
				  [Version]
				, [location_type]
				, [mm_number]
			...
			SET @MessageText = 'Version ' + CAST(@Version AS VARCHAR(20)) + ' created in [npsg].[OdmLotShipSnapshots] with number of records: ' + CAST(@CountInserted AS VARCHAR(20));
			EXEC [CallistoCommon].[stage].[CreateTaskMessage] @TaskId, 'Information', @MessageText;
