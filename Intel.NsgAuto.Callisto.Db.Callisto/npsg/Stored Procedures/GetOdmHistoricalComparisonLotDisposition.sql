...
-- ==================================================================================================================================================
-- Author       : ftianx (modified based on [npsg].[GetOdmComparisonLotDisposition])
-- Create date  : 2021-11-03 10:58:58.698
-- Description  : Get ODM Historical Comparison Lot Disposition
-- Example      : EXEC [npsg].[GetOdmHistoricalComparisonLotDisposition] 'fuhantx', 15
-- ==================================================================================================================================================
CREATE PROCEDURE [npsg].[GetOdmHistoricalComparisonLotDisposition] 
	@UserId VARCHAR(25), 
	@Id INT
AS
BEGIN
	...
	IF ((SELECT COUNT(*) FROM [npsg].[OdmWipSnapshots] WITH (NOLOCK) WHERE [Version] = @OdmWipSnapshotVersion) > 0)
	BEGIN
	...
	BEGIN
		...
		SELECT 
				...
				[category] [varchar],
				[time_entered]
		FROM [npsg].[OdmWipSnapshotsHistory] WITH (NOLOCK) 
		WHERE [Version] = @OdmWipSnapshotVersion
	END
	...
	IF ((SELECT COUNT(*) FROM [npsg].[OdmLotShipSnapshots] WITH (NOLOCK) WHERE [Version] = @LotShipSnapshotVersion) > 0)
	BEGIN
	...
	BEGIN
		...
		SELECT 
				...
				[reticle_wave_id] ,
				[probe_program_rev] 
		FROM [npsg].[OdmLotShipSnapshotsHistory] WITH (NOLOCK) 
		WHERE [Version] = @LotShipSnapshotVersion
	END
	...
	(
		...
		FROM [npsg].[OdmQualFiltersHistory] nqm WITH (NOLOCK)
			...
			LEFT JOIN [npsg].[OdmQualFilterLotDispositionsHistory] ld  WITH (NOLOCK)
