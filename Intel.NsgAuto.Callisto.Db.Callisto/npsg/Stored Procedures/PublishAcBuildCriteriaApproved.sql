...
-- =========================================================================================================================
CREATE PROCEDURE [npsg].[PublishAcBuildCriteriaApproved]
(
...
BEGIN
	...
	BEGIN
		...
		BEGIN TRY
			...
			UPDATE TOP (1) [TREADSTONENPSG].[treadstone].[dbo].[build_rule] SET [lastmodified_by] = [lastmodified_by] WHERE 0 = 1;
		    --UPDATE TOP (1) [npsg].[build_rule_Test] SET [lastmodified_by] = [lastmodified_by] WHERE 0 = 1;
		END TRY
		BEGIN CATCH
	...
	BEGIN
		...
		--BEGIN TRANSACTION
			...
			BEGIN
				...
				FROM [TREADSTONENPSG].[treadstone].[dbo].[build_rule] WITH (NOLOCK)
				--FROM [npsg].[build_rule_Test] WITH (NOLOCK)
				...
				BEGIN
					UPDATE [TREADSTONENPSG].[treadstone].[dbo].[build_rule] SET
					--UPDATE [npsg].[build_rule_Test] SET					
				...
				ELSE IF (@Count = 0)
				BEGIN
					INSERT [TREADSTONENPSG].[treadstone].[dbo].[build_rule]
					--INSERT [npsg].[build_rule_Test]					
					(
						  [design_id]
				...
				DELETE FROM [TREADSTONENPSG].[treadstone].[dbo].[build_rule_condition] WHERE [build_rule_id] = @BuildRuleId;
				
				--DELETE FROM [npsg].[build_rule_Test] WHERE [build_rule_id] = @BuildRuleId;				

				-- insert all condition records for the build_rule_id
				INSERT INTO [TREADSTONENPSG].[treadstone].[dbo].[build_rule_condition]
				--INSERT INTO [npsg].[build_rule_condition_Test]
				(
					  [column_name]
					, [column_operator]
					, [operator]
