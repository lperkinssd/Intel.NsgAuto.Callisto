...
-- =============================================
CREATE PROCEDURE [qan].[CreateOdmQualFilterExceptions] 
...
BEGIN
	...
	DECLARE @MatScenarioId INT = (SELECT [MatVersion] FROM [qan].[OdmQualFilterScenarios]  WITH (NOLOCK) WHERE [Id] = @Id);
    ...
    BEGIN
		...
		INSERT INTO [qan].[OdmQualFilterNonQualifiedMediaExceptions] (
			 [ScenarioId]
			, [OdmId]
			, [DesignId]
			, [SCodeMMNumber]
			...
			FROM [qan].[MAT]  WITH (NOLOCK) WHERE [MatVersion] = @MatScenarioId AND [Media_IPN] = @MediaIpn
			EXCEPT
			SELECT DISTINCT @Id, @OdmIdPlaceHolder, [DesignId], [SCode], [MediaIPN], @LotId 
			FROM [qan].[OdmQualFilters]  WITH (NOLOCK) WHERE [SLot] = @LotId AND [ScenarioId] = @Id
	...
	UPDATE [qan].[OdmQualFilterNonQualifiedMediaExceptions]
		SET [OdmId] = nqm.[OdmId]
	FROM  [qan].[OdmQualFilterNonQualifiedMediaExceptions] ex
		INNER JOIN [qan].[OdmQualFilters] nqm 
			ON ex.[SLot] = nqm.[SLot]
	WHERE nqm.[ScenarioId] = @Id
		AND ex.[ScenarioId] = @Id
		AND ex.[OdmId] = @OdmIdPlaceHolder;

	-- Merge all the non qualified media exceptions into [qan].[OdmQualFilters]  as qualified data records
	DECLARE @QualifiedCategoryId INT = (SELECT oqfc.Id FROM [ref].[OdmQualFilterCategories] oqfc WITH (NOLOCK) WHERE oqfc.[Name] = 'Qualified');
		...
		(
			SELECT
			   ...
			   , exc.[MediaIPN]
			   , exc.[SLot]
			FROM  [qan].[OdmQualFilterNonQualifiedMediaExceptions] exc WITH (NOLOCK)  
