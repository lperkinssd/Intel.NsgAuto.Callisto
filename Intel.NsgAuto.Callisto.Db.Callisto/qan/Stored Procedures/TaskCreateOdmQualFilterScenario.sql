...
-- Example      : EXEC [qan].[TaskCreateOdmQualFilterScenario];
-- Notes        : Copied and tweaked from [qan].[CreateOdmQualFilterScenario]. Every time
...
-- ======================================================================================
CREATE PROCEDURE [qan].[TaskCreateOdmQualFilterScenario]
	  @Id            INT         = NULL OUTPUT
...
BEGIN
	...
	DECLARE @LatestPRFVersion INT = (SELECT ISNULL(MAX([PrfVersion]), 0) FROM [qan].[PRFDCR] WITH (NOLOCK));
	DECLARE @LatestMATVersion INT = (SELECT ISNULL(MAX([MatVersion]), 0) FROM [qan].[MAT] WITH (NOLOCK));
	DECLARE @LatestWIPVersion INT = (SELECT ISNULL(MAX([Version]), 0) FROM [qan].[OdmWipSnapshots] WITH (NOLOCK));
	DECLARE @LatestShipVersion INT = (SELECT ISNULL(MAX([Version]), 0) FROM [qan].[OdmLotShipSnapshots] WITH (NOLOCK));
	DECLARE @LatestManualDispositionsVersion INT = (SELECT ISNULL(MAX([Version]), 0) FROM [qan].[OdmManualDispositions] WITH (NOLOCK));

	BEGIN TRY

		EXEC [CallistoCommon].[stage].[CreateTaskByName] @TaskId OUTPUT, 'Create ODM QF Scenario';

		SET @CountInserted = 0;
		IF (@UserId IS NULL) SET @UserId = [qan].[CreatedByTask](@TaskId);
		...
		DECLARE @LatestScenarioDate DATETIME2(7) = (SELECT MAX([CreatedOn]) FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE [Id] = @LatestScenarioId);
		...
		ELSE
		BEGIN
			SET @DailyId = (SELECT ISNULL([DailyId], 0) + 1 FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE [Id] = @LatestScenarioId);
		END;
		...
		INSERT INTO [qan].[OdmQualFilterScenarios]
		(
			  [PrfVersion]
			, [MatVersion]
		...
		EXEC [CallistoCommon].[stage].[CreateTaskMessage] @TaskId, 'Information', @MessageText;
		EXEC [CallistoCommon].[stage].[UpdateTaskProgress] @TaskId, 50, 'Executing [qan].[CreateOdmQualFilters]';
		EXEC [qan].[CreateOdmQualFilters] @Id, @UserId;

		SELECT @CountInserted = COUNT(*) FROM [qan].[OdmQualFilters] WITH (NOLOCK) WHERE [ScenarioId] = @Id;
		SET @MessageText = 'Number of records inserted into [qan].[OdmQualFilters]: ' + ISNULL(CAST(@CountInserted AS VARCHAR(20)), 'null');
		EXEC [CallistoCommon].[stage].[CreateTaskMessage] @TaskId, 'Information', @MessageText;
	...
	BEGIN CATCH
		...
		SET @MessageText = 'Error occurred in [qan].[TaskCreateOdmQualFilterScenario] / [qan].[CreateOdmQualFilters] at line# ' + CAST(ERROR_LINE() AS NVARCHAR(50)) + ': ' + CAST(ERROR_MESSAGE() AS NVARCHAR(4000));
		...
		IF @Id IS NOT NULL
		BEGIN
			--INSERT INTO [qan].[OdmQualFilterAttemptedScenarios]
			--	   ([AttemptedScenarioId]
			--	   ,[FailedReason]
			--	   ,[AttemptedOn]
			--	   ,[AttemptedBy])
