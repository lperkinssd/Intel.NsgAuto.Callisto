...
-- EXEC [qan].[CreateOdmQualFilters] 50, 'jkurian'
-- =============================================================
CREATE PROCEDURE [qan].[CreateOdmQualFilters]
(
            @ScenarioId INT,
            @UserId varchar(50)
)
...
BEGIN
            ...
            ----DECLARE @QualFilterResultsCount INT = (SELECT COUNT(*) FROM [qan].[OdmQualFilters] oqf WITH (NOLOCK) WHERE oqf.ScenarioId = @ScenarioId);
			...
			SELECT @designFamilyId = Id FROM [ref].[DesignFamilies] WITH (NOLOCK) WHERE Name = 'Optane';
           ...
           , m.[Fab_Facility]
            ...
            BEGIN
                        ...
                        FROM #MediaAttributes mat WITH (NOLOCK)
                                    INNER JOIN [qan].[PRFDCR] prf WITH (NOLOCK)
                                                ON mat.Scode = prf.MM_Number
                                    INNER JOIN [qan].[OdmNameMappings] onm WITH (NOLOCK)
                        ...
                        FROM #ActiveSCodes asco WITH (NOLOCK)
                                    INNER JOIN [qan].[OdmWipBohSnapshots] owbs WITH (NOLOCK)
                        ...
                        FROM #ActiveSLots asl WITH (NOLOCK)
                                    INNER JOIN [qan].[OdmLotShipSnapshots] lss WITH (NOLOCK)
                                    --INNER JOIN [TREADSTONEPRD].treadstone.[dbo].[lot_ship] lss WITH (NOLOCK)
                        ...
                        */
                        ...
                        INSERT INTO [qan].[OdmQualFilters]
                                       ([ScenarioId]
                                       ,[OdmId]
                                       ,[DesignId]
                                       ,[SCode]
                        ...
                        --SELECT *  FROM [qan].[OdmQualFilters] WITH (NOLOCK);
                        ...
                        FROM [qan].[OdmQualFilters] oqf WITH (NOLOCK)
                        ...
                        --SELECT * FROM [qan].[OdmQualFilterLotDispositions] WITH (NOLOCK) 
                        --SELECT COUNT(*) FROM #SLotShipments
                        ...
                        */
                        ...
                        WHERE ISNUMERIC(sls.burn_tape_revision) <> 1
                        ...
                        --PRINT 'Loaded [qan].[OdmQualFilters] with #SLotsWithBadBurnTapeRevision';
                        --SELECT *  FROM [qan].[OdmQualFilters] WITH (NOLOCK);
                        ...
                        SELECT
                                       ...
                                       , 'system'                                                      AS [UpdatedBy]
                        FROM [qan].[OdmQualFilters] oqf WITH (NOLOCK)
                                    INNER JOIN [qan].[OdmQualFilterScenarios] oqfs  WITH (NOLOCK)
                        ...
                        WHILE @@FETCH_STATUS = 0
                                    BEGIN
                                                ...
                                                SET @QualFilterQuery = @QualFilterQuery + [qan].[OdmParseMajorProbeProgRev](@Cur_Major_Probe_Program_Revision);             
                                                -- attach burn tape revision criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseBurnTapeRevision](@Cur_Burn_Tape_Revision);            
                                                -- attach cell  revision criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseCellRevision](@Cur_Cell_Revision);
                                                -- attach fab  Excursion Id criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseFabExcrId](@Cur_Fab_Excr_Id);
                                                -- attach Prod Grade criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseProductGrade](@Cur_Product_Grade);
                                                -- attach fab conv id criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseFabConvId](@Cur_Fab_Conv_Id);
                                                -- attach reticle wave id criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseReticleWaveId](@Cur_Reticle_Wave_Id);
                                                -- attach custom test required criteria
                                                SET @QualFilterQuery = @QualFilterQuery + ' OR ' + [qan].[OdmParseCustomTestingReqd](@Cur_Custom_Testing_Required);
                        ...
                        --PRINT 'Insert non-qualified records into [qan].[OdmQualFilters] ';
                        INSERT INTO [qan].[OdmQualFilters] 
                                    ([ScenarioId]
                                    ,[OdmId]
                                    ,[DesignId]
                        ...
                        INSERT INTO [qan].[OdmQualFilterLotDispositions]
                                    ...
                                    INNER JOIN [qan].[OdmQualFilters] oqf WITH (NOLOCK)
            ...
            --execute [qan].[GetCanUseLot]

            --This step adds the qualified lot list into a table.
            EXECUTE [qan].[CreateOdmQualFilterExceptions] @UserId, @ScenarioId

            -- TAKE PREVIOUS SCENARIO LOT DISPOSITIONS AND MERGE IT INTO CURRENT SCENARIO
            --EXECUTE [qan].[UpdateOdmQualFilterLotDispositions] @UserId, @ScenarioId
            --DECLARE @PrevScenarioId INT = ISNULL(@ScenarioId, 0) - 1;
            DECLARE @PrevScenarioId INT = (SELECT ISNULL(MAX(Id), 0) FROM [qan].[OdmQualFilterScenarios] WHERE Id < @ScenarioId)
            ...
            BEGIN
                        ...
                        SELECT
                                    ...
                                    , ld.[UpdatedOn]
                                    , ld.[UpdatedBy]
                        FROM [qan].[OdmQualFilterLotDispositions] ld WITH (NOLOCK)
                                    LEFT JOIN [qan].[OdmQualFilters] qf 
                        ...
                        MERGE [qan].[OdmQualFilters] AS TRG
                        USING
                        (
                                    SELECT
                                                [LotDispositionId],
                        ...
                        -- then left join it with past lot dispositions ([qan].[OdmQualFilterLotDispositions]) - This will tell us what lot dispositions are not in current scenario from past ones
                        ...
                        SELECT 
                                                ...
                                                , pld.[UpdatedOn]
                                                , pld.[UpdatedBy]
                                    FROM [qan].[OdmQualFilters] qf WITH (NOLOCK) -- current qual filters
                                                LEFT JOIN [qan].[OdmQualFilterLotDispositions] ld -- current lot dispositions
                        ...
                        MERGE [qan].[OdmQualFilterLotDispositions] AS TRG
                        USING
                        (
                                    SELECT
                                                  [LotDispositionId]
			...
			(
				...
				FROM [qan].[OdmQualFilters] qf WITH (NOLOCK)
			...
			DELETE qm 
			FROM [qan].[OdmQualFilterNonQualifiedMediaExceptions] qm WITH (NOLOCK)
			INNER JOIN  [qan].[OdmQualFilterRemovableMedia] rm WITH (NOLOCK)
