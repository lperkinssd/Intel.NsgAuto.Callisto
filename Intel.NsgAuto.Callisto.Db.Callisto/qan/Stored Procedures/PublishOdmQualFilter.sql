...
-- =============================================
CREATE PROCEDURE [qan].[PublishOdmQualFilter]
...
BEGIN
	...
	BEGIN
		SET @ScenarioId = (SELECT MAX(Id) FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK));
	END

    DECLARE @ScenarioDate DateTime2(7) = (SELECT [CreatedOn] FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE Id = @ScenarioId);
	DECLARE @OdmWipSnapshotVersion INT = (SELECT [OdmWipSnapshotVersion] FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE [Id] = @ScenarioId)
