...
-- =============================================
-- Author:		jakemurx
-- Create date: 2020-10-22 17:07:52.653
-- Description:	returns the "active" reviewers for a given request and stages
--				EXEC [qan].[GetProductLabelReviewersWithWork] @VersionId, @StageId, @StageId2, @UserId
...
--
-- =============================================
CREATE PROCEDURE [qan].[GetProductLabelReviewersWithWork] 
...
BEGIN
	...
	IF (@StageId2 IS NOT NULL) BEGIN
		...
		SELECT DISTINCT(R.[Name]) AS [ReviewerName], R.[Email] AS [ReviewerEmail], @VersionId AS [VersionId]
		INTO #Reviewers10
		FROM [qan].[ProductLabelReviewStages] S 
		INNER JOIN [qan].[ProductLabelReviewGroups] G WITH (NOLOCK) ON (G.[ReviewStageId]=S.[Id] AND G.[VersionId]=S.[VersionId])
		INNER JOIN [qan].[ProductLabelReviewers] R WITH (NOLOCK) ON (R.[ReviewGroupId]=G.[Id] AND R.[ReviewStageId]=S.[Id] AND R.[VersionId]=S.[VersionId])
