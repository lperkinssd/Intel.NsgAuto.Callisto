...
BEGIN
	...
	BEGIN
		...
		BEGIN TRY
			...
			--  UPDATE TOP (1) [qan].[build_rule_Test] SET [lastmodified_by] = [lastmodified_by] WHERE 0 = 1;
		END TRY
		BEGIN CATCH
	...
	BEGIN
		...
		--BEGIN TRANSACTION
			...
			BEGIN
				...
				--FROM [qan].[build_rule_Test] WITH (NOLOCK)
				...
				BEGIN
					...
					--UPDATE [qan].[build_rule_Test] SET					
				...
				--DELETE FROM [qan].[build_rule_condition_Test] WHERE [build_rule_id] = @BuildRuleId;			

				-- insert all condition records for the build_rule_id
				INSERT INTO [TREADSTONE].[treadstone].[dbo].[build_rule_condition]
				--INSERT INTO [qan].[build_rule_condition_Test]
				(
					  [column_name]
					, [column_operator]
					, [operator]
