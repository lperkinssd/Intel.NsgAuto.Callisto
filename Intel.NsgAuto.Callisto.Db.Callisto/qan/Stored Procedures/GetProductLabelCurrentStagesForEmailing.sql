...
-- =============================================
-- Author       :	jakemurx
-- Create date  :	2020-10-22 17:05:05.570
-- Description	:	For the specified VersionId, figure out which stage should get 
--					any emails.  This SP uses the qan.ProductLabelReviewStageStates table
...
--					[qan].[GetProductLabelReviewersWithWork] stored proc to get the emails.  This
...
--
--					EXEC [qan].[GetProductLabelCurrentStagesForEmailing] @VersionId, @UserId, @IsUpdate, @StageId1 OUTPUT, @StageId2 OUTPUT
--
-- =============================================
CREATE PROCEDURE [qan].[GetProductLabelCurrentStagesForEmailing] 
...
BEGIN
 ...
 --   DROP TABLE #LEVEL3
	...
	INTO #LEVEL3
	FROM #LEVEL2
	LEFT JOIN [qan].[ProductLabelReviewStageStates] S WITH (NOLOCK) ON (S.[VersionId]=@VersionId AND S.[ReviewStageId]=[SID])
	...
	IF (@MinSeq > 0) BEGIN
		...
		IF (@IsUpdate = 1) BEGIN
			UPDATE [qan].[ProductLabelReviewStageStates] 
			SET [HasEmailSent]=0 
		...
		IF (@IsActive IS NULL OR  @IsActive != 1) BEGIN
			...
			INSERT INTO [qan].[ProductLabelReviewStageStates] 
		...
		IF (@TotalRejected > 0) BEGIN
			...
			FROM [qan].[ProductLabelReviewStageStates] WITH (NOLOCK) 
			WHERE [VersionId]=@VersionId 
			AND [ReviewStageId]=@SID
	...
	--Last step is to update the [ProductLabelReviewStageStates] table
	UPDATE [qan].[ProductLabelReviewStageStates]
	SET [HasEmailSent]=1
