...
--                
--                EXEC [qan].[GetOdmScenarioInfo] @userId,  @scenarioId 
-- ==================================================================================================================================================
CREATE PROCEDURE [qan].[GetOdmScenarioInfo]
(
	@userId               VARCHAR(50)
	, @scenarioId         INT
)
...
BEGIN
	...
	DECLARE @latestLotshipLoadTime datetime2(7) = (SELECT MIN([CreatedOn]) FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE [LotShipSnapshotVersion] = @LotShipSnapshotVersion)
	DECLARE @latestWipLoadTime datetime2(7) = (SELECT MIN([CreatedOn]) FROM [qan].[OdmQualFilterScenarios] WITH (NOLOCK) WHERE [OdmWipSnapshotVersion] = @OdmWipSnapshotVersion)
	DECLARE @latestMatImportTime datetime2(7) = (SELECT MAX([CreatedOn]) FROM [qan].[MAT] WITH (NOLOCK) WHERE [MatVersion] = @MatVersion)
	DECLARE @latestPrfImportTime datetime2(7) = (SELECT MIN([CreatedOn]) FROM [qan].[PRFDCR] WITH (NOLOCK) WHERE [PrfVersion] = @PrfVersion)
