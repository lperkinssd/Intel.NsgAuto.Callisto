...
-- Description : Create records in [qan].[OdmWipBohSnapshots] from the historical data in
--               [TREADSTONEPRD].[treadstone].[odm].[vw_odm_wip_data_history]
-- Notes       : Copied and tweaked from [qan].[TaskCreateOdmQualFilterWipSnapshot]
-- Example     : EXEC [qan].[TaskCreateOdmQualFilterHistoricalWipBohSnapshot];
-- ================================================================================================
CREATE PROCEDURE [qan].[TaskCreateOdmQualFilterHistoricalWipBohSnapshot]
(
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC [CallistoCommon].[stage].[CreateTaskByName] @TaskId OUTPUT, 'Create ODM QF Historical WIP BOH Snapshot';
		DECLARE @Count         INT;
		DECLARE @LatestMatVersion INT = (SELECT MAX([MatVersion]) FROM [qan].[MAT] WITH (NOLOCK));
		...
		FROM [TREADSTONEPRD].[treadstone].[odm].[vw_odm_wip_data_history] ows WITH (NOLOCK) -- from history
		...
		INSERT INTO #odm_history_wip_diff
			...
			SELECT
				...
				, [subcon_name]
				, [intel_part_number]
			FROM [qan].[OdmWipBohSnapshots] WITH (NOLOCK)
		...
		BEGIN
			...
			EXEC [CallistoCommon].[stage].[UpdateTaskProgress] @TaskId, 90, 'Creating a new version and inserting records into [qan].[OdmWipBohSnapshots]';

			INSERT INTO [qan].[OdmWipBohSnapshots] (
				[media_lot_id]
			   ,[subcon_name]
			   ,[intel_part_number]
			   )
			...
			SET @MessageText = 'Created in [qan].[OdmWipBohSnapshots] with number of records: ' + CAST(@Count AS VARCHAR(20));
			EXEC [CallistoCommon].[stage].[CreateTaskMessage] @TaskId, 'Information', @MessageText;
