...
--         EXEC [qan].[GetMATStagesAndReviewers] 11
-- =============================================
CREATE PROCEDURE [qan].[GetMATStagesAndReviewers] 
...
BEGIN
	...
	--DECLARE @GroupId int;
	--SELECT @GroupId=Id FROM [qan].[ReviewStageReviewGroups] WHERE ProcessId=@ProcessId
	--IF (@GroupId is NULL)
	--	return;
	...
	SELECT @CurrentStatusId = [StatusId] FROM [qan].[MATVersions] WITH (NOLOCK) WHERE [Id] = @VersionId
