-- ==========================================================================================================
-- Author       : bricschx
-- Create date  : 2020-11-13 15:40:44.327
-- Description  : Updates an auto checker attribute type
...
-- ==========================================================================================================
CREATE PROCEDURE [qan].[UpdateAcAttributeType]
(
...
BEGIN
	...
	-- begin validation
	SELECT @IdValid = MAX([Id]) FROM [qan].[AcAttributeTypes] WITH (NOLOCK) WHERE [Id] = @Id;
	SELECT @DataTypeIdValid = MAX([Id]) FROM [ref].[AcAttributeDataTypes] WITH (NOLOCK) WHERE [Id] = @DataTypeId;
	...
	BEGIN
		...
		SET @ErrorsExist = 1;
	END
	ELSE IF (EXISTS (SELECT 1 FROM [qan].[AcAttributeTypes] WITH (NOLOCK) WHERE [Name] = @Name AND [Id] <> @Id))
	BEGIN
		SET @Message = 'Name already in use';
		SET @ErrorsExist = 1;
	END
	ELSE IF (EXISTS (SELECT 1 FROM [qan].[AcAttributeTypes] WITH (NOLOCK) WHERE [NameDisplay] = @NameDisplay AND [Id] <> @Id))
	BEGIN
	...
	IF (@ErrorsExist = 0)
	BEGIN
		UPDATE [qan].[AcAttributeTypes] SET
			  [Name] = @Name
	...
	EXEC [qan].[CreateUserAction] NULL, @UserId, @ActionDescription, 'Auto Checker', 'Update', 'AcAttributeType', @Id, NULL, @Succeeded, @Message;
