...
-- =============================================
-- Author:		jakemurx
-- Create date: 2020-10-07 15:46:53.860
-- Description:	Create MAT Snapshots
-- =============================================
CREATE PROCEDURE [qan].[CreateMATSnapshots] 
...
BEGIN
	...
	SELECT @StageVersion = [VersionId] FROM [qan].[MATReviewStages] WHERE [VersionId] = @VersionId
	...
	WHERE RTRS.ReviewTypeId = (SELECT [Id] FROM [ref].[ReviewTypes] WITH (NOLOCK) WHERE [Description] = 'Material Attribute Table Version')
	AND RS.[IsActive] = 1

	-- Insert them into the MATReviewStages table
	INSERT INTO [qan].[MATReviewStages]
		( [Id]
		, [VersionId]
		, [StageName]
		, [DisplayName]
	...
	-- Insert them into the MATReviewGroups table
	INSERT INTO [qan].[MATReviewGroups]
		( [Id]
		, [VersionId]
		, [ReviewStageId]
		, [GroupName]
	...
	-- And insert them into the MATReviewers table.
	INSERT INTO [qan].[MATReviewers]
		( [VersionId]
		, [ReviewStageId]
		, [ReviewGroupId]
		, [Name]
